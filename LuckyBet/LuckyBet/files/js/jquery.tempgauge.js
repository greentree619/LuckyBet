	...
	$.fn.tempGauge = function(options) {
		...
		function createTempGauge(gauge){
			...
			var canvas = document.createElement("canvas"),
                ...
                currentTemp =  opts.defaultTemp;

			canvas.width = opts.width;
			canvas.height = opts.height + opts.labelSize;
			...
			fillTempGauge(ctx, 5, padding + padding, opts.width, opts.height - padding * 6, percentage);
			strokeTempGauge(ctx, 5, padding + padding, opts.width, opts.height - padding * 6);
			if (opts.showLabel) {
				drawLabel(ctx, canvas.width / 2, canvas.height - opts.labelSize / 5, currentTempText);
			}

			if(opts.showScale) {
				drawScale(ctx, 0, padding * 2, opts.width, opts.height - padding, 0, percentage, opts.maxTemp, opts.minTemp);
			}

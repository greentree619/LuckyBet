...
function ShowGift(strMsg) {
    ...
    if (idx >= 0) {
        ...
        jQuery("#watchGift_pub .stream .chatList").append(strVal);
        jQuery("#watchGift_pub").scrollTop(1E10);
...
function sendMsg() {
    ...
    if (txt.length > 0) {
        PostAjax("/Ajax/chat_action.ashx", "cmd=send&txt=" + jQuery(".to-text input").val() + "&chl=" + ChannelInfo);
...
//{{make full height.
jQuery("#all_chat").height((jQuery(window).height() - 77) + "px");
//}}make full height.
